...
export abstract class AbstractImage<PATH extends PossiblePath,
    ...
    // @ts-ignore
    #getImages<I extends PossibleImages = PossibleImages, >(image: I,): PossibleImages_Array<I, PATH>
    #getImages<I1 extends PossibleImages = PossibleImages, I2 extends PossibleImages = PossibleImages, >(images: readonly [I1, I2,],): readonly [PossibleImages_Array<I1, PATH>, PossibleImages_Array<I2, PATH>,]
    #getImages<I1 extends PossibleImages = PossibleImages, I2 extends PossibleImages = PossibleImages, I3 extends PossibleImages = PossibleImages, >(images: readonly [I1, I2, I3,],): readonly [PossibleImages_Array<I1, PATH>, PossibleImages_Array<I2, PATH>, PossibleImages_Array<I3, PATH>,]
    ...
    #getImages(images: PossibleImages | readonly PossibleImages[],) {
        if (images instanceof Array)
            return images.map(image => this.#getImages(image));
    ...
    public get waitingImages(): WaitingImages<PATH> {
        return this.#waitingImages ??= this.#getImages(AbstractImage.#WAITING_IMAGE);
    }

    public get tauntImages(): TauntImages<PATH> {
        return this.#tauntImages ??= this.#getImages(AbstractImage.#TAUNT_IMAGE);
    }

    public get downImages(): DownImages<PATH> {
        return this.#downImages ??= this.#getImages(AbstractImage.#DOWN_IMAGE);
    }

    public get walkImages(): WalkImages<PATH> {
        return this.#walkImages ??= this.#getImages(AbstractImage.#WALK_IMAGES);
    }

    public get runningImages(): RunningImages<PATH> {
        return this.#runningImages ??= this.#getImages(AbstractImage.#RUNNING_IMAGES);
    }

    public get swimmingImages(): SwimmingImages<PATH> {
        return this.#swimmingImages ??= this.#getImages(AbstractImage.#SWIMMING_IMAGES);
    }

    public get jumpImages(): JumpImages<PATH> {
        return this.#jumpImages ??= this.#amountOfImagesOnJump === 1
            ? [this.#getImages(AbstractImage.#JUMP_IMAGE_1), EMPTY_ARRAY, EMPTY_ARRAY,]
            : this.#getImages(AbstractImage.#JUMP_IMAGES);
    }

    public get fallingAfterJumpImages(): FallingAfterJumpImages<PATH> {
        return this.#fallingAfterJumpImages ??= this.#getImages(AbstractImage.#FALLING_AFTER_JUMP_IMAGE);
    }

    public get turningImages(): TurningImages<PATH> {
        return this.#turningImages ??= this.#getImages(AbstractImage.#TURNING_IMAGE);
    }

    public get climbingImages(): ClimbingImages<PATH> {
        return this.#climbingImages ??= this.#getImages(AbstractImage.#CLIMBING_IMAGES);
    }

    public get goalPoleImages(): GoalPoleImages<PATH> {
        return this.#goalPoleImages ??= this.#getImages(AbstractImage.#GOAL_POLE_IMAGES);
    }
