...
export abstract class AbstractImageContainer
    implements Image {
    ...
    private __getSpecificImage(expectEmpty: boolean, gameStyle: GameStyles, theme: Themes, time: Times,): readonly string[] {
        ...
        if (theme === Themes.GROUND && time === Times.DAY)
            return imagesFromMap ?? this.#defaultImages.get(gameStyle)! ?? AbstractImageContainer.#EMPTY_ARRAY;
        return expectEmpty
            ? imagesFromMap ?? AbstractImageContainer.#EMPTY_ARRAY
            : imagesFromMap ?? this.#defaultImages.get(gameStyle) ?? AbstractImageContainer.#EMPTY_ARRAY;
    }
