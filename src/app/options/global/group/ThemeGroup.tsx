...
export default class ThemeGroup
    ...
    protected override _renderElement(element: Themes, option: GlobalAppOption<GlobalThemeOption>, [hasSMM2Selected, isDisabledNight,]: readonly [boolean, boolean,], onClickCallback: | OnClickCallback | null,): ReactElement {
        return <div key={`option container (${element.englishName})`} id={`${element.englishNameInHtml}-option-container`} className="btn-group-vertical" role="group">{
            hasSMM2Selected
                ...
                : ThemeGroup.__renderElementWithOnlyDay(element, option,)
        }</div>;
    }


    private static __renderThemeImage(element: Themes, value: boolean, isDisabled: boolean, onClickCallback: () => void,): ReactElement {
    ...
    private static __renderElementWithOnlyDay(element: Themes, option: GlobalAppOption<GlobalThemeOption>,): ReactElement {
        const optionValue = option.get;
        const value = optionValue.dayValue;

        return this.__renderThemeImage(element, value, false, () => option.set(optionValue.reverseDayValue),);
    }

    private static __renderElementWithDayAndNight(element: Themes, option: GlobalAppOption<GlobalThemeOption>, isDisabledNight: boolean,): ReactElement {
        const optionValue = option.get;
        ...
        return <>
            {this.__renderThemeImage(element, value, isDisabledNight, () => option.set(optionValue.set(value)),)}
